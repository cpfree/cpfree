...
结语
这不止是一份面试清单，更是一种“被期望的责任”，因为有无数个待面试者，希望从这系列文章中，找出通往期望公司的“钥匙”。上面的每道选题都是结合我自身的经验，于千万个面试题中，经过艰辛的两周，一个题一个题筛选出来再校对好答案和格式做出来的，面试的答案也是再三斟酌，生怕误人子弟。一道题目事小，影响他人的“仕途”才是大过，所以如有纰漏，还请读者朋友们在评论区不吝指出。
...
o	java运行机制
o	 先编译后执行；
o	先编译为二进制字节码文件（二进制）.class文件；
o	由jvm统一执行字节码文件；
o	java是跨平台的
o	 原因：
o	（1）java由jvm运行，不是直接运行在os上的；
o	（2）java先编译为二进制.class文件，这个文件是统一的；
o	（3）.class文件由jvm根据os类型进行执行；
o	class文件是统一的， jvm是统一的，os不统一被jvm屏蔽了；所有java语言是跨平台；
o	java回收机制（GC）
•	java语法基础
o	java的注释规范
	注释的分类
...
•	默认值为null
•	运算符
o	java类型及运算符示例 算数运算符
o	整型数算的结果还是整型，如果运算结果中含有小数部分，则小数部分直接舍弃；【注意，不是四舍五入】
	+ 、- 、*、 /、 %、 +=、 -= 、/= 、%= 
o	位运算
o	位运算的操作数是 二进制的 0 或 1 ； 
o	        位运算操作的数在计算机中都是以补码的形式存在的； ★★★
o	        所有的数在计算机中都是以补码的形式存在的；   ★★★  【整数的原码和补码、反码是一样的】
o	        补码：是为了让负数在cpu的加法运算器中正常运算的表示数的方式；【补码就是将减法变为加法的方式；也是负数在计算机中表示的方式】
o	              0 1 
o	              1000 0001   原码   第一位为符号位  1 表示负数  0表示整数；
o	                符号位不变，其他各位取反+1；
o	              1111 1111   补码   表示的是 -1 
	&amp;、|、^、<<、>>、>>>
o	自增、自减运算
o	运算符的位置分为：先增、后增
o	                        先减、后减
...
o	
o	        自增自减运算有两种值：
o	            一个值是： 表达式的值； 这个值一般参与运算的； 先增时表达式的值是+1的；后增时表达式的值不加+1；
o	            一个值是： 操作数变量自身的值；  这个值一般不参与运算，只是变量自身值的改变； 不管先增还是后增，都会+1；
	++、--
o	比较运算
o	比较运算的结果都是 boolean 类型；
	 > 、< 、==、 !=、 >=、 <=
o	逻辑运算符
o	 解析：
o	            非是一元运算符；与、或是两元运算符
o	            逻辑运算参与运算的操作数都是布尔类型，运算结果也是布尔类型；
o	            逻辑运算一般经常和比较运算进行搭配使用；
o	            逻辑运算一般经常作为判断表达式进行出现；
o	
o	        非：true变false；false变true；
o	        与：只有true与true结果为true；其他结果为false；【会产生运算短路】
o	        或：只有false或false结果为false；其他结果为true；【会产生运算短路】
o	        
o	        因为与运算和或运算的特点会造成逻辑运算的短路；    
o	        与短路：多个连续的与运算时，从左往右，如果有一个操作数为false，则整个表达式为false；后面的运算不再执行，造成了与短路；
o	        或短路：多个连续的或运算时，从左往右，如果有一个操作数为true，则整个表达式为true；后面的运算不再执行，造成了或短路；
	&amp;&amp;、||、！
o	条件运算符
o	是唯一的一个三元运算符；需要三个操作数；
o	        判定表达式?结果表达式1:结果表达式2;
o	        如果判定表达式为true，则结果表达式1作为整个表达式的结果；反之，则结果表达式2作为整个问号表达式的结果；
o	        问号表达式可以嵌套使用，等价于if-else；
	条件表达式？值1：值2
•	程序结构
o	程序结构代码示例 顺序结构
	从上到下，从左到右依次执行
...
	switch - case -default
•	break
•	结束循环体，直接跳到循环体之后的代码继续顺序执行；
•	continue
•	结束本次循环，跳过continue关键字周后循环体部分，直接开始下一次新的循环；
...
	按地址（引用）传递
	 实参的地址传递给形参；
	                形参值发生了变化，会影响实参的值，也就是形参变化实参也会跟着变化；
o	函数的重载
o	当一个类中有多个同名的函数时，就构成了函数的重载；
o	        函数的重载条件：要求函数名相同，参数列表不同；
o	            参数个数不同；参数类型不同；参数顺序不同；
o	        注意：
o	            返回值类型不同不构成重载；修饰符不同不构成重载；
o	            重载的方法在调用时，由实参列表决定调用那个重载方法；也就是，调用的实参列表和形参列表要求具有对应关系；【参数类型排列关系】
o	变长（可变）参数
o	 函数在定义的时候，可以在定的参列表中 将 最后的参数设置变长类型的参数【个数不固定的参数，0个，1个，多个都可以】
o	        变长参数在一个函数定义中至多只能有一个，且只能放在参数列表的最后；
o	        变长参数本质上是一个数组；
o	        变长参数也可以构成重载，如果非变长参数和变长参数都可以满足方法调用，则优先调用非变长参数的方法；
o	        变长参数在使用上和数组一样；
o	        变长参数和数组类型的区别：数组类型必须有参数传递，不能是0个；变长参数可以是0个；
o	        变长参数传递实参采用 逗号 分割形式，可以写 0 个 1个 多个；
o	        变长参数的实参可以是数组的形式进行传递；也可以是 逗号 分割形式；比较灵活；
o	        反之数组类型形参，不能使用变长参数的形式进行传参；
•	数组
o	一维数组
o	二维数组
o	多维数组
•	面向对象
o	面向对象的详细概念 什么是面向对象？
o	面向对象的思维方式： 做什么事情【解决什么问题】？首先考虑的是 找谁去做？ 这个 谁 就是对象；
o	
o	面向对象编程的方法：一开始就创建一个类；然后通过抽象填充 属性  行为 ，这样就定义完了类，然后类和类相互调用就是面向对象的编程； 
o	        
o	面向过程的思维方式： 做什么事情【解决什么问题】？首先考虑的是 如何去做？ 这个 如何 就是过程；
	 面向对象映射的是人类社会的思维方式
o	类
o	也就是在面向对象的思想中，使用类来规范定义对象； 有了类之后，可以使用new操作符创建这个类的实例对象；
	 是对对象的规范定义
o	面向对象的核心
	抽象
	抽象的时候要从业务角度进行； 
	面向对象的抽象是从业务角度进行，将共性抽取出来；
	共性主要从两个维度去描述：属性、行为         
...
•	 将抽象的结果封装到类中
	继承
	java的继承是单继承方式，使用extends关键字实现；一个类只能有一个直接父类；
	【C++是多继承】【单继承是java简单性的一种体现】
	java中的顶级父类是Object类；
...
	 公开的，任何类都可访问
•	构造方法
•	构造方法是类的行为方法中的一类特殊的方法；
•	用于在类进行创建生产实例对象时而调用的方法；
o	构造方法与普通方法的区别
...
o	成员变量
o	成员变量可以使用访问控制修饰符修饰；
o	成员变量声明之后有默认值；不进行初始化也可以直接使用；【根据类型而定】
o	成员变量的作用域范围是整个类内部；【有点类似C语言全局变量】
o	成员变量的生命周期是伴随在类的实例对象的生命周期而存在的；
	在类中定义的变量
•	局部变量
•	局部变量不能使用访问控制修饰符修饰；
...
o	基本类型：不允许在修改【值不能改】
o	引用类型：不允许修改引用地址，存储空间中的内容可以修改； 【地址的值不能改；地址的存储空间内容可以改】
•	static final 修饰的成员变量 叫做静态常量 ； 命名规则 字母全部大写用_分割； 静态常量要求声明时就要进行初始化；  
o	abstract
o	使用abstract修饰的类使抽象类；
o	含有抽象方法的类是抽象，抽象类不一定非要含有抽象方法；
o	抽象方法是之定义方法名而没有方法体；【方法定义时去掉 {} 就是抽象方法】
o	如果一个类继承了抽象类，则这个类要么将父类的抽象方法通过重写进行实现；要么这个子类继续作为一个抽象类存在；
o	抽象类和普通类区别在于使用abstract关键字进行修饰，或者是含有了抽象方法；
o	因为抽象类含有抽象的内容，所以不能使用 new 直接创建实例对象；【new 分配内存空间；abstract是没有实现的，无法分配内存；矛盾】
	抽象的
•	abstract可以修饰类、方法；        abstract修饰的类叫做抽象类；修饰的方法叫做抽象方法；
o	 interface
o	解析：
o	接口需要使用interface进行定义；
o	接口中只有抽象方法，没有其他普通方法【jdk8-】；【考试以这个说法为准】
o	jdk8+？以后可以使用default关键字在接口中定义一个带方法体的普通方法，default方法也只能使用public修饰，public可以省略；
o	接口中定义的方法都是 public abstract 修饰的方法，其中 public abstract 关键字可以全部或者部分省略；【不能用其他的访问控制修饰符】
o	接口中定义的变量都是 public static final 修饰的变量【常量】，其中 public static final 可以全部或者部分省略；【不能用其他的访问控制修饰符】
o	接口和抽象类一样不能使用new直接创建；
o	如果子类实现【implements】接口则子类需要全部实现接口中的抽象方法；
	接口
•	接口是一组行为规范的定义； 接口是一种特殊的抽象类；特殊到需要使用interface重新定义；
•	支持多继承（实现）
o	类多继承接口：一个类【class】可以继承多个接口；使用implements关键进行继承【实现】   
o	接口多继承接口：一个接口可以继承多个接口；使用extends关键字进行继承；
•	Util
o	Object
...
•	java对象的执行顺序：
o	（1）执行父类的静态语句块；        （2）执行子类的静态语句块；        （3）执行父类非静态语句块；        （4）执行父类的构造方法；        （5）执行子类的非静态语句块        （6）执行子类的构造方法
...
•	泛型
o	 泛型是对某一类未知的类型的统称
o	在集合中使用泛型来表示集合可以处理的类型范围
o	要求在能够明确泛型的地方一定要使用明确的泛型，可以减少不必要的非检查性异常
•	IO流
o	字节流
	InputStream
•	FileInputStream
...
o	调用wait方法
•	锁池
o	调用notify、notifyAll方法
•	死亡（销毁）
	使用时必须使用synchronized（同步），使方法同步或代码块同步
•	 状态图地址
•	超链接  死锁
o	 形成的条件（必须全部满足才能形成死锁）
